...
export default function TwitchAuthButton({
  ...
  const handleTwitchAuth = () => {
    if (!clientId) {
      alert(
        "Twitch Client ID is not configured. Please add TWITCH_CLIENT_ID to your .env file.",
      );
      return;
    }
    ...
    if (!redirectUri) {
      alert(
        "Redirect URI is not configured. Please add TWITCH_REDIRECT_URI to your .env file.",
      );
      return;
    }
    ...
    if (!scope) {
      alert(
        "Scopes are not configured. Please add TWITCH_SCOPE to your .env file.",
      );
      return;
    }
    ...
    window.location.href = `https://id.twitch.tv/oauth2/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${scope}&state=${state}`;
  };
